...
export class Tab3Page {
  ...
  exibirAtores(Atores: IAtores){
    ...
    this.router.navigate(['Atores-detalhe'], navigationExtras); //chama o navigate e pega a q vc vai abrir com os dados do Atores, por isso navigationExtras (ele ta sendo declarado e define um state parametro para a navegação) 
  }
  async exibirAlertaFavorito(Atores: IAtores) {
    const alert = await this.alertController.create({

      header: 'Meus Favoritos',
      message: 'Deseja realmente favoritar o Atores?',
      buttons: [
        {
          text: 'Não',
          role: 'cancel',
          handler: () => {
            Atores.favoritos=false;
          }
        }, {
          text: 'Sim, favoritar.',
          handler: () => {
            Atores.favoritos=true;
 ...
 async apresentarToast() {
  const toast = await this.ToastController.create({
    message: 'Atores adicionado aos favoritos...',
    duration: 2000,
    color: 'success',
    position: 'top'
  });
